...
def print_tree(node: Any, indent:Int = 0): Unit =
    val prefix = " " * indent
    node match {
        case ("number", value) => println(s"$prefix${node(1)}")
        case ("binop", p[@@] => println(s"$prefix${node(1)}"); print_tree(node(2), indent + 2)
        case "binop" => println(s"$prefix${node(1)}"); print_tree(node(2), indent + 2); print_tree(node(3), indent + 2)
    }
